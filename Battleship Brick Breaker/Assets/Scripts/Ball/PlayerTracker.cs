    ...
    private void OnCollisionEnter(Collision other)
    {
        switch (other.collider.tag)
        {
            case "OutZone":
                ...
                break;
            case "Paddle":
                if ((int)currentOwner == 1 && !other.gameObject.GetComponentInParent<PaddleController>().Player1)
                {
                    SwitchCurrentOwner();
                }
                else if ((int)currentOwner == 2 && other.gameObject.GetComponentInParent<PaddleController>().Player1)
                {
                    SwitchCurrentOwner();
                }
                break;
            case "Brick":
                ...
                BrickHealth brickHealth = other.gameObject.GetComponent<BrickHealth>();
                if (brickHealth != null)
                {
                    if (currentOwner == CurrentOwner.player1)
                    {
                        brickHealth.TakeDamge(damage, true);
                    }
                    else
                    {
                        brickHealth.TakeDamge(damage, false);
                    }
                }
